...
$PageContent .= "<li class='nav'><a onclick=\"return ".$jsFun."('$prev');\" style=\"text-decoration:none;background-color: rgb(0, 0, 0)\">Prev</a></li>";
}
//
...
$PageContent .= "<li class='nav'><a onclick=\"return ".$jsFun."('$next');\" style=\"background-color: rgb(0, 0, 0)\"><span style=\"text-decoration:none\">Next</span></a></li>";
}
...
function getLogodetails1($compid,$path){
$querylogo=db_query("SELECT LG_Logo FROM  ".TABLE_LOGO." WHERE  `LG_UserFk` ='".$compid."'");
$Fetchlogo=mysql_fetch_array($querylogo);
$logo=$path.$Fetchlogo['LG_Logo'];
if($Fetchlogo['LG_Logo']!='')
  $scr = $Fetchlogo['LG_Logo'];  
else
   $scr = 'images/default/no_logo.png';
return $scr;
}
...
function getSectordetails1($secid){
$querysector=db_query("SELECT S_Name FROM  ".TABLE_SECTOR." WHERE  `Id` ='".$secid."'");
$Fetchsector=mysql_fetch_array($querysector);
$sectorname =  $Fetchsector['S_Name'];
return $sectorname;
}
...
function getRelatedSearchBestdeals($searchkey,$userCity,$sectorid){
...
if($userCity!=''){
$wherec = "AND RGT_City= $userCity"; 	
}
else{
  $wherec = ""; 	  
}
//echo "SELECT RGT_PK FROM ".TABLE_REGISTRATION." WHERE  RGT_Status=1 $wherec $querysectormatch";
$querycitymatch1=db_query("SELECT RGT_PK FROM ".TABLE_REGISTRATION." WHERE  RGT_Status=1 ".$wherec." $querysectormatch");
...
//echo "SELECT Id FROM ".TABLE_ADMINPRODUCT." WHERE ProductName='$searchkey'";
...
$searchquery=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='1'");
$searchquery1=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='1'");
$searchquery2=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='1'");
$searchquery3=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='1'");


//echo "SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con";
//echo "SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con";
//echo "SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con";
...
while($fetchquery=mysql_fetch_array($searchquery)){ 
 ...
 $sectordisplay = get_sector_id($fetchquery['PS_User_Fk']);
...
$prodser  = $fetchquery['PS_Id'];
	

echo '<div class="list1">';     
echo '<div class="list1a">';      
...
echo '" >'.$Compnamedisp.'</a><span>-since'.$yearofestablishment[2].'</span></h3>';
 
 echo '<div class="list1b">';                              
 echo '<div class="list-border" id="containera">';                                  
   echo '<div class="list-border1" id="sidebar">';                                    
  echo 'Company Type:';                                     
  echo '</div>';                                     
  echo '<div class="list-border2" id="content">';                                     
  echo getSectordetails($sectordisplay);                                     
     echo '</div>';                                   
    echo '</div>';                                   
     echo '</div>';                               
     ...
     echo '<div class="list1b">';                            
      ...
      echo '<div class="list-border2" id="content">';                                 
      echo '<ul>';                                     
      echo '<li><img src="images/star.png" width="10%"/></li>';                                         
      echo '<li><img src="images/star.png" width="10%"/></li>';                                         
       echo '<li><img src="images/star.png" width="10%"/></li>';                                        
       echo '<li><img src="images/star1.png" width="10%"/></li>';                                        
         echo '<li><img src="images/star1.png" width="10%"/></li>';                                     
         echo '<li><img src="images/star1.png" width="10%"/></li>'; 
          echo '</ul>';                                 
          echo '</div>';                             
...
function getRelatedServiceBestdeals($searchkey,$userCity,$sectorid){
...
//echo $citymatchdata;
$queryprodname=db_query("SELECT Id FROM ".TABLE_ADMINSERVICE." WHERE ServiceName LIKE '$searchkey%'");
//echo "SELECT Id FROM ".TABLE_ADMINPRODUCT." WHERE ProductName='$searchkey'";
while($fetchprodid=mysql_fetch_array($queryprodname)){
$matchingids.=$fetchprodid['Id'].',';
}
...
{ 
// Based On Keyword, Display
$findproductmatch     = get_Search_Id(TABLE_ADMINSERVICE, "Id", "ServiceName", $searchkey);
//echo $findproductmatch.'</br>';
$findkeywordmatch     = get_Search_Id(TABLE_PRODUCTSERVICE, "PS_Id", "PS_Keyword", $searchkey);
//echo $findkeywordmatch.'</br>';
$finddisplaymatch     = get_Search_Id(TABLE_PRODUCTSERVICE, "PS_Id", "PS_Display", $searchkey);
...
$searchquery=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='2'");
$searchquery1=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='2'");
$searchquery2=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='2'");
$searchquery3=db_query("SELECT * FROM ".TABLE_PRODUCTSERVICE." WHERE PS_User_Fk IN (".$citymatchdata.") $con AND PS_Mode='2'");
...
function getRelatedSearchBestdeals1($searchkey,$userCity){
...
if($fetchquery['PS_CoverImg']!=''){ $imgsrc = $fetchquery['PS_CoverImg']; } else { $imgsrc = 'images/default/no_image.png'; }

$sectordisplay = get_sector_id($fetchquery['PS_User_Fk']);

$sectorname=getSectordetails1($sectordisplay);

$data = <<<APPCOMP
 <div class="pic">       
<img  width="164" height="157" src="{$imgsrc}"/> 
<span class="pic-caption rotate-in">  
<h4>Company Type:</h4>
<p><a onclick="bestdealresult('$orginal','$prodser','',1);"> $sectorname </a></p> 
...
function getRelatedServiceBestdeals1($searchkey,$userCity){
...
function get_sector_id($productId){
$querysectorid=db_query("SELECT RGT_Sector FROM ".TABLE_REGISTRATION." WHERE RGT_PK=$productId ");
$fetchsectorid=mysql_fetch_array($querysectorid);
return $fetchsectorid['RGT_Sector'];   
}
...
function getKeywordCompListFromSearchedCompany($searchkey1,$type2,$usercity,$sectorid)
{
...
if($sectorid!='')
  ...
  $querysectormatch ='';
//echo $searchkey1;
$searchval = explode(" ",$searchkey1);
$searchkey = $searchval[0];
...
//echo $searchsql;
$sqltot="SELECT * FROM  ".TABLE_REGISTRATION." WHERE  $searchsql RGT_Status=1 AND RGT_Type=2 $querycitymatch  $queryareamatch $querysectormatch";
...
$queryrelated=db_query($sqltot);    
$countresult=mysql_num_rows($queryrelated);
if($countresult>0){ 
    
while($Fetchrelated=mysql_fetch_array($queryrelated)){
    
$yearofestablishment = explode('-',$Fetchrelated['RGT_YrofEstablish']);    
$orginal = $Fetchrelated['RGT_CompName'];
$count=strlen($Fetchrelated['RGT_CompName']);
if($count>25){
$displaydata=(substr($Fetchrelated['RGT_CompName'],0,25));
$displaydata.='...';
}
else {
$displaydata=$Fetchrelated['RGT_CompName'];
...
$SelectSector=db_query("Select Id,S_Name From ".TABLE_SECTOR." WHERE Status=1 AND Id='".$Fetchrelated['RGT_Sector']."'");
$sector_name = db_fetch_array($SelectSector);
...
echo '" >'.$displaydata.'</a><span>-since'.$yearofestablishment[2].'</span></h3>'; 
...
function getKeywordCompListFromSearchedCompany1($searchkey1,$type2,$usercity,$sectorid)
{
...
$imgsrc = getLogodetails1($Fetchrelated['RGT_PK'],$path); 
$data = <<<APPCOMP
 <div class="pic">       
<img  width="164" height="157" src="{$imgsrc}"/> 
<span class="pic-caption rotate-in">  
<h4>Company Type:</h4>
<p><a onclick="searchResult('$orginal','',1,'$cmp');"> {$sector_name['S_Name']} </a></p>
<h4>Company Review</h4>
<div id="grid">
<ul>
<li><img src="images/star.png" width="10%"/></li>
